...
repos. In Dart projects the placed `test/dna/dna_test.dart` calls gg_dna's
`runDnaTest()` directly. This package brings the _same engine, byte for
byte_ to TypeScript projects: the io-free engine core of gg_dna 5.x is
...
gg_dna's golden-update semantics:
...
The package bundles gg_dna's own `dna/` folder (`dist/base_dna`) as the
...
typescript/runDnaTest.ts Node host callbacks (node:fs, git) + the outcome
                         semantics of gg_dna's run_dna_test.dart.
typescript/index.ts      Public API + hand-written TypeScript types.
build.dart               dart compile wasm + bundles gg_dna's dna/ folder
                         into typescript/generated/base_dna.
```

Only the io-free parts of gg_dna are imported
(`package:gg_dna/src/engine/instantiate.dart`,
`package:gg_dna/src/util/dna_fs.dart`) — the gg_dna barrel exports
...
as `bridge-wasm.ts`) and copies gg_dna's `dna/` folder to
...
## Relationship to gg_dna and the bridge template

- **gg_dna 5.x** (Dart) owns the engine. Since 5.0 the engine core is
  ...
  (`../../ggsuite/gg_dna`); releases pin the hosted `gg_dna: ^5.0.0`.
